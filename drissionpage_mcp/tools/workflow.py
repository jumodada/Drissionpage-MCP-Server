...
class BrowserOpenAndSnapshotInput(ToolInput):
    ...
    wait_condition: WaitCondition = Field(
        default="",
        description="Optional wait condition before snapshot capture.",
    )
    selector: str = Field(
        default="",
        description="Selector used by element/text wait conditions.",
    )
    wait_value: str = Field(
        default="",
        description="Value used by URL/text wait conditions.",
    )
    ...
    include_console: bool = Field(
        default=False,
        description="Also include recent console messages from the page.",
    )
...
class BrowserExtractLinksInput(ToolInput):
    ...
    include_text: bool = Field(
        default=True,
        description="Include bounded visible link text.",
    )
    ...
    absolute_urls: bool = Field(
        default=True,
        description="Return absolute URLs in the url field when possible.",
    )
...
class FormFillPreviewInput(ToolInput):
    ...
    fields: Dict[str, Any] = Field(
        ...
        min_length=1,
        description=(
            "Map of field selectors, ids, names, labels, or placeholders to values. "
            "Values are redacted from output by default."
        ),
    )
    submit: bool = Field(
        default=False,
    ...
    redact_values: bool = Field(
        default=True,
        description="Redact submitted field values from tool output.",
    )
    ...
    def _reject_submit(cls, value: bool) -> bool:
        if value:
            raise ValueError("form_fill_preview never submits; use element_click after review")
        return value
...
@define_tool(
    ...
    description=(
        "Open a URL, optionally wait for a page condition, then return a bounded "
        "snapshot plus optional form and console context."
    ),
...
async def browser_open_and_snapshot(
    ...
    args: BrowserOpenAndSnapshotInput,
    ...
    async with tool_errors(
        ...
        result = await tab.open_and_snapshot(
            url=args.url,
        ...
        response.add_code("page.get(url); page.run_js(<bounded snapshot workflow>)")
        response.add_result(
...
@define_tool(
    ...
    title="Extract Links",
    description=(
        "Extract bounded link data from the current page with optional origin "
        "filtering and URL normalization."
    ),
...
async def browser_extract_links(
    context: "DrissionPageContext",
    args: BrowserExtractLinksInput,
    response: "ToolResponse",
) -> None:
    """Extract links from the active page."""

    async with tool_errors(response, "Failed to extract page links"):
        tab = context.current_tab_or_die()
        result = await tab.extract_links(
            selector=args.selector,
            limit=args.limit,
        ...
        response.add_code("page.run_js(<bounded link extraction script>)")
        response.add_result(
...
@define_tool(
    ...
    description=(
        "Prefill matched form controls without submitting. Returns a redacted "
        "review payload and requires explicit confirmation before submit."
    ),
...
async def form_fill_preview(
    ...
    async with tool_errors(response, "Failed to fill form preview"):
        tab = context.current_tab_or_die()
        result = await tab.form_fill_preview(
            form_selector=args.form_selector,
            fields=args.fields,
            redact_values=args.redact_values,
        )
        response.add_code("page.run_js(<safe form prefill script; no submit>)")
        response.add_result(
            f"Prepared {result['filled_count']} form fields for review",
            **result,
        )
        response.set_include_snapshot(True)


tools = [browser_open_and_snapshot, browser_extract_links, form_fill_preview]
