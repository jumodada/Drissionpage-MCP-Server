...
class WaitUrlInput(BaseModel):
    ...
    url_pattern: str = Field(..., description="Substring or pattern expected in the current URL")
    timeout: int = Field(default=10, description="Timeout in seconds")
...
async def wait_for_element(
    ...
    try:
        ...
        response.add_code(f"page.wait.ele_loaded({args.selector!r}, timeout={args.timeout!r})")
...
async def wait_for_url(
    ...
    try:
        ...
        response.add_result(f"URL matched '{args.url_pattern}' within {args.timeout} seconds")
