...
class ScreenshotInput(BaseModel):
    ...
    path: str = Field(default="", description="Optional local file path to save screenshot")
...
async def screenshot(
    ...
    try:
        tab = context.current_tab_or_die()
        screenshot_data = await tab.screenshot(path=args.path or None, full_page=args.full_page)

        response.add_code(
            f"page.get_screenshot(path={args.path!r}, full_page={args.full_page!r})"
        )
        if args.path:
            response.add_result(f"Screenshot saved to: {screenshot_data}")
        else:
