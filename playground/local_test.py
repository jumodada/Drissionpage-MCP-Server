...
class LocalMCPTester:
    ...
    def setup_mocks(self):
        ...
        mock_mcp = Mock()
        mock_mcp.types = Mock()
        mock_mcp.types.TextContent = lambda type="text", text="": Mock(
            type=type, text=text
        )
        mock_mcp.types.ImageContent = (
            lambda type="image", data="", mimeType="image/png": Mock(
        ...
        if importlib.util.find_spec("DrissionPage") is not None:
    ...
    def setup_drissionpage_mock(self):
        ...
        class MockChromiumPage:
            ...
            def ele(self, selector):
                ...
                mock_element.input = lambda text: logger.info(
                    f"⌨️ Mock: Input '{text}' to {selector}"
                )
    ...
    while True:
        try:
            ...
            if command == "quit" or command == "exit":
                ...
                print(
                    "❓ Unknown command. Try 'list', 'test <tool>', 'scenarios', or 'quit'"
                )
