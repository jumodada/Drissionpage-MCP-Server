...
```bash
python -m pip install -U "drissionpage-mcp>=0.8.2" "mcp>=1.0.0,<2"
drissionpage-mcp doctor
```
...
## Task Completion / Browser-Owned Capabilities 0.8.2 Checks
...
python -m tests.evals.task_completion_benchmark \
  --iterations 10 \
  --output benchmark-results/0.8.2-task-completion.json
```
